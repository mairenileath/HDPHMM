...
max_no_states<-as.integer(args[2])
no_iterations<-as.integer(args[3])
c<-as.integer(args[4])
c_hap<-as.integer(args[5])
conc_param_states<-as.integer(args[6])
conc_param_transitions<-as.integer(args[7])
resample_haplotypeSwitching = T
if(length(args)>=8){
	resample_haplotypeSwitching<-as.logical(args[8])
}
...
saveDir = paste("/lustre/scratch103/sanger/dw9/HDPHMM/allchrs_outputs_maxz",max_no_states,"_iters",no_iterations,"_c",c,"_chap",c_hap,"_concParamStates",conc_param_states,"_concParamTransitions",conc_param_transitions,"_resampleHap",resample_haplotypeSwitching,sep="")
...
for(chr in 1:23){	
	...
	settings$trial=chr
	BAFdata[[chr]]=read.table(paste("/nfs/cancer_archive02/dw9/haplotyped_data/",samplename,"_filtered_0109_ALL_noRefLoci/",samplename,"_chr",toString(chr),"_heterozygousMutBAFs_haplotyped.txt",sep=""),sep="\t",header=T)

	BAFs[[chr]]=BAFdata[[chr]][,3]
	pos=BAFdata[[chr]][,2]/1000000
	#150212 - winsorise data
	BAFs[[chr]] = madWins(BAFs[[chr]],2.5,25)[[1]]

	logRdata[[chr]]=read.table(paste("/nfs/cancer_archive02/dw9/haplotyped_data/",samplename,"_filtered_0109_ALL_noRefLoci/",samplename,"_chr",toString(chr),"_heterozygousLogRs.txt",sep=""),sep="\t",header=T)

	logRs[[chr]]=logRdata[[chr]][,3]
	#150212 - winsorise data
	logRs[[chr]] = madWins(logRs[[chr]],2.5,25)[[1]]

	#data1=list(obs=array(BAFs,c(1,length(BAFs))))#1-D
	obs=rbind(BAFs[[chr]],logRs[[chr]])
	data1[[chr]]=obs

	datalen = datalen + length(BAFs[[chr]])
}
...
if(length(args)>=9){
	settings$lastSave=as.integer(args[9])
...
print(unique(S$stateSeq[1,1][[chr]]$z))
print(S$theta$mu)

for(chr in 1:23){
	png(filename=paste(settings$saveDir,"/chr",chr_name,"_segmented.png",sep=""),width=1500,height=1000)
	par(mfrow=c(2,1))
	segmentedBAFs = S$theta$mu[cbind(1,S$stateSeq[1,1][[chr]]$z,1)]
	plot(pos,BAFs[[chr]],type="p",col="red",pch=".",ylim=c(0,1),main=paste(samplename," chr ",chr_name,sep=""),xlab="position (Mb)",ylab="BAF")
	points(pos,segmentedBAFs,col="green",pch=".",cex=2)

	segmentedlogRs = S$theta$mu[cbind(2,S$stateSeq[1,1][[chr]]$z,1)]
	plot(pos,logRs[[chr]],type="p",col="red",pch=".",ylim=c(-1,1),main=paste(samplename," chr ",chr_name,sep=""),xlab="position (Mb)",ylab="log(R)")
	points(pos,segmentedlogRs,col="green",pch=".",cex=2)
	dev.off()

	outdata=cbind(BAFdata[[chr]][,1:2],segmentedBAFs)
	...
	outdata=cbind(logRdata[[chr]][,1:2],segmentedlogRs)
	names(outdata)[3]=samplename
	write.table(outdata,file=paste(settings	$saveDir,"/chr",chr_name,"_segmentedLogRs.txt",sep=""),sep="\t",row.names=F,quote=F)
	write.table(data.frame(state=S$stateSeq[1,1][[chr]]$z),file=paste(settings	$saveDir,"/chr",chr_name,"_states.txt",sep=""),sep="\t",row.names=F,quote=F)
}
q(save="no")