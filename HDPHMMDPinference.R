HDPHMMDPinference<-function(data_struct,model,settings,restart){
	...
	if(restart==1){
        ...
        if("filename" %in% names(settings)){
            filename = paste(settings$saveDir,'/',settings$filename,'iter',toString(n),'trial',toString(settings$trial),".Rdata",sep="")
        }else{
            filename = paste(settings$saveDir,'/HDPHMMDPstats','iter',toString(n),'trial',toString(settings$trial),".Rdata",sep="")
		}
