sample_dist <-function(stateCounts,hyperparams,model){
	...
	for (j in 1:Kz){
		...
		# self-transition:
		kappa_vec[j] = kappa0
		#020212 give preference to haplotype block switch over CNV segment switch. We should use a hyperparameter instead of a fixed value
		if (j %% 2 == 0){
			#kappa_vec[j-1] = kappa0/1000
			kappa_vec[j-1] = 25
		}else{
			#kappa_vec[j+1] = kappa0/1000
			kappa_vec[j+1] = 25
